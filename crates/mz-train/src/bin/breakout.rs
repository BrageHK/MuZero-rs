...
fn main() {
    ...
    let mut surface =
        unsafe { softbuffer::Surface::new(&context, &window) }.expect("create surface");
    ...
    event_loop.run(move |event, _, control_flow| {
        ...
        if input.update(&event) {
            ...
            let action = if input.key_held(VirtualKeyCode::Left)
            ...
            if diff > 5 * FRAME_DURATION {
                ...
                println!(
                    "Warning: skip of {}s occured",
                    (diff - 5 * FRAME_DURATION).as_secs_f64()
                );
            }
